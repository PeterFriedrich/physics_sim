...
| 2026-09-24 | **Phase 3 fills in Physics 20** (owner approved the 9-sim plan): motion graphs, river crossing, universal gravitation, connected masses, energy on a track, satellite orbits, pendulum, air-column resonance, Doppler. Motion graphs are slider-driven, not drag-driven, so readouts are exact; the track car is the one stepped motion, and its readouts come from the energy equation, not the step. Protected by `test_motion1d_area_under_vt_equals_displacement`, `test_track_friction_brings_ball_to_rest`, `test_orbits_kepler_constant_same_for_every_radius`. | SPEC_phase3.md §3 |
| 2026-09-24 | **Each course's sims use that course's data sheet; transcribed in `docs/DATA_SHEET.md`.** The sheets disagree on Earth's mass (Physics 30: 5.97 × 10²⁴ kg; Physics 20: 5.98 × 10²⁴ kg), so Physics 20's gravitation and orbits use 5.98 (owner's call), giving g = 9.83 N/kg at the surface and a geostationary radius of 4.23 × 10⁷ m. Rejected: one value for both courses, which would make a correct Physics 20 hand answer look wrong. Protected by `test_constants_earth_mass_per_course_sheet`, `test_gravitation_surface_field_from_physics_20_sheet`. | DATA_SHEET.md §3 |
| 2026-09-24 | **Phase 4 finishes Physics 30 with sheet-only physics** (owner approved all 7): mass spectrometer, potential difference, double slit/grating, energy levels, EM spectrum, wire in a field, nuclear decay. The induction slot became a wire in a field, because emf = BLv is not on the sheet but F = Il⊥B is. Hydrogen levels and nuclide masses are shown as *given* data. The small-angle λ = xd/(nl) is shown next to the exact λ = d sin θ/n. Protected by `test_massspec_only_selected_speed_gets_through`, `test_interference_grating_needs_exact_form`, `test_reactions_charge_and_nucleons_balance`, `test_wireforce_only_perpendicular_length_counts`. | SPEC_phase4.md §3 |
